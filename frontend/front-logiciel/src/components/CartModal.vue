<template>
  <div>
    <b-button id="show-btn" variant="success" @click="showModal()"
      >Panier</b-button
    >
    ...
    <b-modal ref="cart" hide-footer>
      <p v-for="article in currentOrder.orderList" :key="article.titre">
        ...
        <b-button
          ...
          variant="danger"
          block
          @click="deleteArtcile(article.id)"
          inline
          >Supprimer</b-button
        >
      </p>
      <p>Total : {{ currentOrder.totalPrice }} €</p>
      <b-button class="mt-3" variant="success" block @click="takeOrder()"
        >Commander</b-button
      >
      <b-button class="mt-3" variant="danger" block @click="deleteOrder()"
        >Tout supprimer</b-button
      >
    </b-modal>
  </div>
</template>
